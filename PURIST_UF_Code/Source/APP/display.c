...
#define PACK_FORMAT_DUT       "%03dT %04dL"
#define UV_FORMAT_DUT         "%03dT %04dHr"
#define PACK_FORMAT_FRE       "%03dJ %04dL"
#define UV_FORMAT_FRE         "%03dJ %04dHr"
#define PACK_FORMAT_SPA       "%03dD %04dL"
#define UV_FORMAT_SPA         "%03dD %04dHr"
#define PACK_FORMAT_ITA       "%03dG %04dL"
#define UV_FORMAT_ITA         "%03dG %04dHr"
#define PACK_FORMAT_POR       "%03dD %04dL"
#define UV_FORMAT_POR         "%03dD %04dHr"
...
//strings
char *strTankEmpty[DISP_LAN_NUM] =
{
    "Tank Epmty",
    "水箱空",
    "Tank Leer",
...
char *strWash[DISP_LAN_NUM] =
{
	"CLEAN UF/DISINFECTION",
	"清洗 UF",
	"REINIGUNG UF/DESINFEKTION",
    "NETTOYAGE UF/DESINFECTION",
	"LIMPIEZA UF/desinfecion",
	"PULIZIA UF/DISINFEZIONE",
	"LIMPEZA UF/desinfeccao",
};

char *strRecirculation[DISP_LAN_NUM] =
{
    "Recirculation",
    "循 环 ",
    "Rezirkulation",
...
char *strTocOxidate[DISP_LAN_NUM] = 
{
	"TOC Oxidate",
	"TOC 氧化",
	"TOC Oxidate",
...
char *strPressureRelief[DISP_LAN_NUM] = 
{
    "Pressure Relief",
    "泄 压",
    "Druck ablassen",
...
/*
×′ì?    ????·§E7|?-?··§E5|è???·§E6|    3??′·§E8   |×?íaμ?N  |±?C3   |′??D?÷I2|′??D?÷I3|′??D?÷I4
?íD÷           OFF        |            OFF  |      OFF          |           OFF          |      OFF        |     OFF    |      OFF        |  OFF           |OFF
?-?·           OFF        |            ON   |      OFF          |           OFF          |      ON         |     ON |      ON         |  OFF           |ON
è???           ON         |            OFF  |      ON           |           OFF          |      ON         |     ON |      ON         |  ON            |ON
?¨á?è???ON        |            OFF  |      ON           |           OFF          |      ON         |     ON |      ON         |  ON            |ON
D1?1           OFF        |            OFF  |      ON           |           OFF          |      OFF        |     OFF    |      OFF        |  OFF           |OFF
TOC3??′     OFF       |            ON   |      OFF          |           OFF          |      OFF        |     ON |      ON         |  OFF           |ON
TOC???ˉ     OFF       |            ON   |      OFF          |           ON           |      ON         |     ON |      ON         |  OFF           |ON
...
void Disp_Calc_Conductivity(int iChl,int iconduct, int itemp)
{
  ...
  {
    tempResistivity = 0.000000003326*(pow(valueRES,3))+ 0.000003586*(pow(valueRES,2)) + 0.0297*valueRES;
	tempResistivity = tempResistivity*(1.0 + 0.02*(25 - tempTemperature));
	Display.iConductivity[iChl] = (tempResistivity * Display.cfg.cfg1.CELLCONSTANT[iChl]);

	//tempTemperature = 25;//T Calculation	
    //Display.iTemperature[iChl] = tempTemperature*1000; // float 2 integer conversion
  }
...
double Disp_Calc_TOC_BASE(int iChl,int iconduct, int itemp)
{
 // double a = 0;
  double valueTMP = itemp*1.0/1;
  double valueRES = iconduct*1.0/1;
  double tempResistance,tempTemperature;
  ...
  if (iChl == 2)
  {
    //B25 (25C) = (1+(25-T)*0.03)*B
    tempB25 = (1+(25-tempTemperature)*0.03)*valueRES;
...
void Disp_Calc_TOC_PEAK(int iChl,int iconduct, int itemp, int temp_B)
{
 // double a = 0;
  ...
  double tempResistance,tempResistivity,tempTemperature,tempP25;
  ...
  if (iChl == 2)
  {
    ...
    //P25 (25C) = (1+(25-T)*0.03)*(P-B)
    tempP25 = (1+(25-tempTemperature)*0.03)*(valueRES-valueB);
	
    //TOC = 2.221*E-11*P25 4 - 7.767*E-08*P25 3 + 1.126*E-04*P252 + 4.07*E-03*P25 + 3.1.
    tempResistivity = 2.221*e11*pow(tempP25,4) - 7.767*e8*pow(tempP25,3) + 1.126*e4*pow(tempP25,2) + 4.07*e3*tempP25 + 3.1;
    Display.iConductivity[iChl] = abs(tempResistivity*Display.cfg.cfg1.CELLCONSTANT[iChl]);
    
    if(Display.iConductivity[iChl]<=1000)
	{
	   Display.iConductivity[iChl] = 1000;
	}
...
{
    char *text;
    //LCD_Clear(Display.usBackColor);//20161108
    text = strTankEmpty[Display.cfg.cfg1.language];
#if 0    
    if (DISP_LAN_ENGLISH == Display.cfg.cfg1.language
		||  DISP_LAN_GERMANY == Display.cfg.cfg1.language
    ||  DISP_LAN_FRENCH  == Display.cfg.cfg1.language
    ||  DISP_LAN_SPANISH == Display.cfg.cfg1.language
    ||  DISP_LAN_ITALIAN == Display.cfg.cfg1.language
    ||  DISP_LAN_PORTUGUESE == Display.cfg.cfg1.language)
    {
        text = "Empty Tank";
    }
    else
...
void Disp_ShowCheckState(int xoff,int yoff)
{
    ...
    case CHECK_SHOW_PACK: // show pack
		...
		{
			strcpy((char *)Config_buff,"Tausch U PACK");
		}
        else if(DISP_LAN_FRENCH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.U PACK");
		}
        else if(DISP_LAN_SPANISH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.U PAQUETE");
		}
        else if(DISP_LAN_ITALIAN == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"cambio U PACK");
		}
		else if(DISP_LAN_PORTUGUESE == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.U PACK");
		}
        else
    ...
    case CHECK_SHOW_UV: // show uv
		...
		{
			strcpy((char *)Config_buff,"Tausch UV Lampe");
		}
        else if(DISP_LAN_FRENCH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.LAMPE UV");
		}
        else if(DISP_LAN_SPANISH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.lampara UV");
		}
        else if(DISP_LAN_ITALIAN == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"cambio LAMPA UV");
		}
		else if(DISP_LAN_PORTUGUESE == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"EXCH.UV LAMPA");
		}
        else
        {
    ...
    case CHECK_SHOW_FILTER: // show filter
		...
		{
			strcpy((char *)Config_buff,"Tausch Endfilter");
		}
        else if(DISP_LAN_FRENCH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"FILTER EXCH.FINAL");
		}
        else if(DISP_LAN_SPANISH == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"FILTRO EXCH.FINAL");
		}
        else if(DISP_LAN_ITALIAN == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"cambio FILTRO FINALe");
		}
		else if(DISP_LAN_PORTUGUESE == Display.cfg.cfg1.language)
		{
			strcpy((char *)Config_buff,"FILTER EXCH.FINAL");
    ...
    case CHECK_SHOW_TANK:
        switch(Display.cfg.cfg1.language)
        {
          ...
          case DISP_LAN_GERMANY:
              strcpy((char *)Config_buff,"LEER TANK");
              break;
          case DISP_LAN_FRENCH:
              strcpy((char *)Config_buff,"VIDE CITERNE");
              break;
          case DISP_LAN_SPANISH:
              strcpy((char *)Config_buff,"VACIO TANQUE");
              break;
          case DISP_LAN_ITALIAN:
              strcpy((char *)Config_buff,"VUOTO SERBATOIO");
              break;
          case DISP_LAN_PORTUGUESE:
              strcpy((char *)Config_buff,"VAZIO TANQUE");
              break;
          default:
    ...
    case CHECK_SHOW_PRODUCT:
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            strcpy((char *)Config_buff,"DISTRIBUTION < S.P.");
            break;
        case DISP_LAN_SPANISH:
            strcpy((char *)Config_buff,"DISTRIBUCION < S.P.");
            break;
        case DISP_LAN_ITALIAN:
            strcpy((char *)Config_buff,"DISTRIBUZIONE < S.P.");
            break;
        case DISP_LAN_PORTUGUESE:
            strcpy((char *)Config_buff,"DISTRIBUICAO < S.P.");
            break;
        default:
    ...
    case CHECK_SHOW_PRODUCT1:
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            strcpy((char *)Config_buff,"Apport DISTRIBUTION > S.P.");
            break;
        case DISP_LAN_SPANISH:
            strcpy((char *)Config_buff,"Entrada DISTRIBUCION >  S.P.");
            break;
        case DISP_LAN_ITALIAN:
            strcpy((char *)Config_buff,"Ingresso DISTRIBUZIONE >  S.P.");
            break;
        case DISP_LAN_PORTUGUESE:
            strcpy((char *)Config_buff,"Entrada DISTRIBUICAO >  S.P.");
            break;
        default:
    ...
    case CHECK_SHOW_TEMPERATURE1:
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            strcpy((char *)Config_buff,"Apport TEMP > S.P.");
            break;
        case DISP_LAN_SPANISH:
            strcpy((char *)Config_buff,"Entrada TEMP > S.P.");
            break;
        case DISP_LAN_ITALIAN:
            strcpy((char *)Config_buff,"Ingresso TEMP > S.P.");
            break;
        case DISP_LAN_PORTUGUESE:
            strcpy((char *)Config_buff,"Entrada TEMP > S.P.");
            break;
    ...
    case CHECK_SHOW_UF_CLEAN:
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            strcpy((char *)Config_buff,"UF Lavage");
            break;
        case DISP_LAN_SPANISH:
            strcpy((char *)Config_buff,"UF Enjuague");
            break;
        case DISP_LAN_ITALIAN:
            strcpy((char *)Config_buff,"UF Risciaquo");
            break;
        case DISP_LAN_PORTUGUESE:
            strcpy((char *)Config_buff,"UF Nivelado");
            break;
        default:
...
void Disp_DisplayIdlePage(void)
{
    ...
    // display text info
    text = strReady[Display.cfg.cfg1.language];
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
int Disp_Sensors_Item(int chl,int big,int mask,int xoff,int yoff)
{
    ...
    if (mask & 0x1)
    {
        ...
        else
        {
           ...
           // Disp_Int2floatFormat(fus,1,3,buf);
            Disp_Int2floatFormat(fus,3,1,buf);
        ...
        if (big) Disp_SelectFont(&font48);  
         ...
         {
             curFont->DrawText(xoff-45,yoff,Config_buff,BLACK,usBackColor); //35
         }
     ...
     //   curFont->DrawText(xoff,yoff,Config_buff,BLACK,usBackColor);
         ...
         {
            curFont->DrawText(xoff-30,yoff,Config_buff,BLACK,usBackColor);//35
         }
    ...
    if (mask & 0x2)
    {
		...
		switch(Display.cfg.cfg1.language)
            ...
            case DISP_LAN_PORTUGUESE:
                ...
                break;
            default : // DISP_LAN_GERMANY
                sprintf((char *)Config_buff,"温度     %s℃",buf);
                break;
            }
    ...
    {    
	   ...
	   {
	   	Disp_Int2floatFormat_toc((Display.iConductivity[chl]+5000)/1000,3,0,buf);
	   	sprintf((char *)Config_buff,"TOC      -- ppb");
        curFont->DrawText(xoff-40,yoff-30,Config_buff,BLACK,usBackColor);
        yoff += 30;
	}
    }
...
void Disp_UpdateIdlePage(int sensor)
{
    ...
    if (sensor)
    {
         // Disp_Sensors(SENSOR_COORD_X,SENSOR_COORD_Y,7);
...
void Disp_DisplayIdleFlushPage(void)
{
    ...
    strcpy(text, strFlushUf[Display.cfg.cfg1.language]);
...
void Disp_CirculationInitPage(void)
{
    ...
    // display text info
    text = strRecirculation[Display.cfg.cfg1.language];
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
void Disp_DisplayUFCleanInitPage(void)
{
    ...
    //Disp_SelectFont(&font48);  
#endif
    text = strFlush[Display.cfg.cfg1.language];
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
void Disp_DisplayPreUFCleanInitPage(void)
{
    ...
    // display info
    text = strPrepare[Display.cfg.cfg1.language];
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
void Disp_TOCFlushPage(void)
{
    ...
    // display text info
    text = strTocFlush[Display.cfg.cfg1.language];
   ...
   if (Display.spec.toc.ucIdx < Display.spec.toc.ucActionCnt )
   {
       ...
       case DISPALY_SUB_PAGE_TOC_FLUSH:
           ...
           Display.TgtState.ucSub  = DISPLAY_SUB_STATE_TOC_FLUSH;
            ...
            {
              sys_timeout(30*1000,SYS_TIMER_ONE_SHOT,0,Disp_toc_cb,&Display,&Display.to4TOC);
            }
			else
			{
...
{   
    ...
    //Display.usBackColor = WHITE;
    LCD_Clear(Display.usBackColor);
	text = strVol[Display.cfg.cfg1.language];
#if 0
    if (DISP_LAN_ENGLISH == Display.cfg.cfg1.language)
    {
...
void Disp_NtwInitPage(void)
{
    ...
    text = strProduct[Display.cfg.cfg1.language];
#if 0
    if (DISP_LAN_ENGLISH == Display.cfg.cfg1.language)
    {
        text = "Product";
...
void Disp_DecInitPage(void)
{
    ...
    // display text info
    text = strPressureRelief[Display.cfg.cfg1.language];
#if 0    
    if (DISP_LAN_ENGLISH == Display.cfg.cfg1.language)
    {
...
void Disp_userSetDrawPack(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
		    ...
		    break;
		case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"PACK:%d Tage %dL    ",tmp,Display.info.PACKLIFEL);
		    break;
		case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"PACK:%d jours %dL    ",tmp,Display.info.PACKLIFEL);
		    break;
		case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"PACK:%d dias %dL    ",tmp,Display.info.PACKLIFEL);
		    break;
		case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"PACK:%d giorni %dL    ",tmp,Display.info.PACKLIFEL);
		    break;
		case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"PAQUETE:%d dias %dL  ",tmp,Display.info.PACKLIFEL);
		    break;
		default:
...
void Disp_userSetDrawFilter(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
		    ...
		    break;
		case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"FILTER:%d Tage   ",tmp);
		    break;
		case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"FILTRE:%d jours   ",tmp);
		    break;
		case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"FILTRO:%d dias   ",tmp);
		    break;
		case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"FILTRO:%d giorni   ",tmp);
		    break;
		case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"FILTRO:%d dias   ",tmp);
		    break;
...
void Disp_userSetDrawUV(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
		    ...
		    break;
		case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"UV:%d Tage %dHr   ",tmp,Display.info.UVLIFEHOUR/(3600/UV_PFM_PEROID));
		    break;
		case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"UV:%d jours %dHr   ",tmp,Display.info.UVLIFEHOUR/(3600/UV_PFM_PEROID));
		    break;
		case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"UV:%d dias %dHr   ",tmp,Display.info.UVLIFEHOUR/(3600/UV_PFM_PEROID));
		    break;
		case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"UV:%d giorni %dHr   ",tmp,Display.info.UVLIFEHOUR/(3600/UV_PFM_PEROID));
		    break;
		case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"UV:%d dias %dHr   ",tmp,Display.info.UVLIFEHOUR/(3600/UV_PFM_PEROID));
		    break;
...
void Disp_userSetDrawFlow(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	switch(Display.cfg.cfg1.language)
	{
		...
		case DISP_LAN_FRENCH:
		    ...
		    break;
		case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"FLUJO:%s L/min  ",buf);
		    break;
		case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"PORTATA:%s L/min  ",buf);
		    break;
		case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"FLUXO:%s L/min  ",buf);
		    break;
		default:
...
void Disp_userSetDrawUnit(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
		    ...
		    break;
		case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"EINHEIT:%s    ",Display.cfg.cfg2.UNIT ? UNIT_TYPE_1 : UNIT_TYPE_0);
            break;
		case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"UNIT:%s    ",Display.cfg.cfg2.UNIT ? UNIT_TYPE_1 : UNIT_TYPE_0);
		    break;
		case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"UNIDAD:%s    ",Display.cfg.cfg2.UNIT ? UNIT_TYPE_1 : UNIT_TYPE_0);
		    break;
		case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"UNITA:%s    ",Display.cfg.cfg2.UNIT ? UNIT_TYPE_1 : UNIT_TYPE_0);
		    break;
		case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"UNIT:%s    ",Display.cfg.cfg2.UNIT ? UNIT_TYPE_1 : UNIT_TYPE_0);
		    break;
...
void Disp_userSetDrawTime(int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_CHINESE:
			...
			break;
		case DISP_LAN_GERMANY:
			strcpy((char *)Config_buff,"ZEIT:");
			break;
        case DISP_LAN_FRENCH:
			strcpy((char *)Config_buff,"Temps:");
			break;
		case DISP_LAN_SPANISH:
			strcpy((char *)Config_buff,"TIEMPO:");
			break;
		case DISP_LAN_ITALIAN:
			strcpy((char *)Config_buff,"TEMPO:");
        case DISP_LAN_PORTUGUESE:
...
void Disp_userSetDrawLanguage(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_GERMANY:
        ...
        break;
    case DISP_LAN_FRENCH:
        strcpy((char *)Config_buff,"LANGUAGE/LANGUE :FRENCH");
        break;
    case DISP_LAN_SPANISH:
        strcpy((char *)Config_buff,"LANGUAGE/IDIOMA :SPANISH");
        break;
    case DISP_LAN_ITALIAN:
        strcpy((char *)Config_buff,"LANGUAGE/LINGUA :ITALIAN");
        break;
    case DISP_LAN_PORTUGUESE:
        strcpy((char *)Config_buff,"LANGUAGE/IDIOMA:PORTUGUESE");
        break;
    default:
...
void Disp_userSetDrawUFWash(int xoff,int yoff,u16 charColor,u16 bkColor)
{
     ...
     strcpy((char *)Config_buff,strWash[Display.cfg.cfg1.language]);
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
{
    strcpy((char *)Config_buff,strFlushUf[Display.cfg.cfg1.language]);
#if 0
    switch(Display.cfg.cfg1.language)
    {
    case DISP_LAN_ENGLISH:
...
void Disp_userSetDrawFeedQuality(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_GERMANY:
        ...
        break;
    case DISP_LAN_FRENCH:
        strcpy((char *)Config_buff,"VALEUR INPUT:");
        break;
    case DISP_LAN_SPANISH:
        strcpy((char *)Config_buff,"ENTRADA:");
        break;
    case DISP_LAN_ITALIAN:
        strcpy((char *)Config_buff,"INGRESSO:");
        break;
    case DISP_LAN_PORTUGUESE:
        strcpy((char *)Config_buff,"ENTRADA:");
        break;
...
void Disp_userSetDrawSerialNo(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_GERMANY:
        ...
        break;
    case DISP_LAN_FRENCH:
        sprintf((char *)Config_buff,"NUMERO DE SERIE:%s",Display.cfg.cfg3.SERIALNO);
        break;
    case DISP_LAN_SPANISH:
        sprintf((char *)Config_buff,"NUMERO DE SERIE:%s",Display.cfg.cfg3.SERIALNO);
        break;
    case DISP_LAN_ITALIAN:
        sprintf((char *)Config_buff,"NUMERO DI SERIE:%s",Display.cfg.cfg3.SERIALNO);
        break;
    case DISP_LAN_PORTUGUESE:
        sprintf((char *)Config_buff,"NUMERO DE SERIE:%s",Display.cfg.cfg3.SERIALNO);
        break;
...
void Disp_engSetDrawCellConstant(int iChl,int xoff,int yoff,u16 charColor,u16 bkColor)
{
	...
	case DISP_LAN_FRENCH:
#ifdef UF_FUNCTION        
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELLULE.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON DE CELLULE.IN:");
#elif defined PRO
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELLULE.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON DE CELLULE.IN:");
#else
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELLULE:");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"CON DE CELLULE.TOC:");
#endif  
	break;
    case DISP_LAN_SPANISH:
#ifdef UF_FUNCTION        
        if (0 == iChl )strcpy((char *)Config_buff,"CON CELULAR.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON CELULAR.EN:");
#elif defined PRO
        if (0 == iChl )strcpy((char *)Config_buff,"CON CELULAR.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON CELULAR.EN:");
#else
        if (0 == iChl )strcpy((char *)Config_buff,"CON CELULAR :");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"CON CELULAR.TOC:");
#endif  
	break;
    case DISP_LAN_ITALIAN:
#ifdef UF_FUNCTION        
        if (0 == iChl )strcpy((char *)Config_buff,"COS DI CELLA.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"COS DI CELLA.IN:");
#elif defined PRO
        if (0 == iChl )strcpy((char *)Config_buff,"COS DI CELLA.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"COS DI CELLA.IN:");
#else
        if (0 == iChl )strcpy((char *)Config_buff,"COS DI CELLA:");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"COS DI CELLA.TOC:");
#endif  
	break;
    case DISP_LAN_PORTUGUESE:
#ifdef UF_FUNCTION        
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELULA.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON DE CELULA.IN:");
#elif defined PRO
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELULA.UP:");
        if (1 == iChl )strcpy((char *)Config_buff,"CON DE CELULA.IN:");
#else
        if (0 == iChl )strcpy((char *)Config_buff,"CON DE CELULA :");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"CON DE CELULA.TOC:");
#endif  
	break;
    default:
#ifdef UF_FUNCTION        
...
void Disp_engSetDrawTempConstant(int iChl,int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_FRENCH:
#ifdef UF_FUNCTION        
        if (0 == iChl) strcpy((char *)Config_buff,"TEMP CONSTANT.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"TEMP CONSTANT.IN:");
#elif defined PRO
        if (0 == iChl) strcpy((char *)Config_buff,"TEMP CONSTANT.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"TEMP CONSTANT.IN:");
#else
        if (0 == iChl) strcpy((char *)Config_buff,"TEMP CONSTANT :");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"TEMP CONST.TOC:");
#endif  
    ...
    case DISP_LAN_SPANISH:
#ifdef UF_FUNCTION        
        if (0 == iChl) strcpy((char *)Config_buff,"CON TEMP.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"CON TEMP.EN:");
#elif defined PRO
        if (0 == iChl) strcpy((char *)Config_buff,"CON TEMP.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"CON TEMP.EN:");
#else
        if (0 == iChl) strcpy((char *)Config_buff,"CON TEMP :");
#endif
#ifdef TOC            
        if (2 == iChl) strcpy((char *)Config_buff,"CON TEMP.TOC:");
#endif  
	break;
    case DISP_LAN_ITALIAN:
#ifdef UF_FUNCTION        
        if (0 == iChl) strcpy((char *)Config_buff,"TEM COSTANTE.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"TEM COSTANTE.IN:");
#elif defined PRO
        if (0 == iChl) strcpy((char *)Config_buff,"TEM COSTANTE.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"TEM COSTANTE.IN:");
#else
        if (0 == iChl) strcpy((char *)Config_buff,"TEM COSTANTE:");
#endif
#ifdef TOC  
	    if (2 == iChl) strcpy((char *)Config_buff,"TEM COST.TOC:");
#endif  
	break;
...
#ifdef UF_FUNCTION        
        if (0 == iChl) strcpy((char *)Config_buff,"CON TEMP.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"CON TEMP.IN:");
#elif defined PRO
        if (0 == iChl) strcpy((char *)Config_buff,"CON TEMP.UP:");
        if (1 == iChl) strcpy((char *)Config_buff,"CON TEMP.IN:");
#else
...
void Disp_engSetDrawProductRs(int chl,int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_PORTUGUESE:
#ifdef UF_FUNCTION        
        if (0 == chl) sprintf((char *)Config_buff,"RES PROD.S.P.UP:%s"UNIT_TYPE_0,buf);
        if (1 == chl) sprintf((char *)Config_buff,"RES PROD.S.P.DI:%s"UNIT_TYPE_1,buf);
#elif defined PRO
        if (0 == chl) sprintf((char *)Config_buff,"RES PROD.S.P.UP:%s"UNIT_TYPE_0,buf);
        if (1 == chl) sprintf((char *)Config_buff,"RES PROD.S.P.DI:%s"UNIT_TYPE_1,buf);
#else
        if (0 == chl) sprintf((char *)Config_buff,"RES PROD.S.P:%s"UNIT_TYPE_0,buf);
#endif
		break;
    default:
#ifdef UF_FUNCTION        
...
void Disp_engSetDrawPack(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
			...
			break;
        case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"PACK TAUSCH:");
			break;
        case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"ECHANGE DU PACK:");
			break;
        case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"CAMBIO PAQUETE:");
			break;
        case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"DURATA PACK:");
			break;
        case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"PACK VIDA:");
			break;
		default:
    ...
    if (Display.ucCurCol < PACK_SET_NUMS)
    {
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            sprintf((char *)Config_buff,PACK_FORMAT_FRE,Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_SPANISH:
            sprintf((char *)Config_buff,PACK_FORMAT_SPA,Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_ITALIAN:
            sprintf((char *)Config_buff,PACK_FORMAT_ITA,Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_PORTUGUESE:
            sprintf((char *)Config_buff,PACK_FORMAT_POR,Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;    
        }    
    ...
    else
    {
        ...
        case DISP_LAN_ENGLISH:
            ...
            break;
        case DISP_LAN_GERMANY:
            sprintf((char *)Config_buff,"%dT %dL    ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;        
        case DISP_LAN_FRENCH:
            sprintf((char *)Config_buff,"%dJ %dL   ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_SPANISH:
            sprintf((char *)Config_buff,"%dD %dL   ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_ITALIAN:
            sprintf((char *)Config_buff,"%dG %dL  ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        case DISP_LAN_PORTUGUESE:
            sprintf((char *)Config_buff,"%dD %dL    ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;    
...
void Disp_engSetDrawFilter(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    {
    case DISP_LAN_ENGLISH:
        sprintf((char *)Config_buff,"FILTER LIFE:%dD    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_GERMANY:
        sprintf((char *)Config_buff,"FILTER TAUSCH:%dT    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_FRENCH:
        sprintf((char *)Config_buff,"CHANGEMENT DU FILTRE:%dJ   ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_SPANISH:
        sprintf((char *)Config_buff,"DURACION FILTRO:%dD    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_ITALIAN:
        sprintf((char *)Config_buff,"DURATA FILTRO:%dG  ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_PORTUGUESE:
        sprintf((char *)Config_buff,"FILTRO DE VIDA:%dD   ",Display.cfg.cfg2.FILTERLIFE);
        break;
    default:
...
void Disp_engSetDrawUV(int xoff,int yoff,u16 charColor,u16 bkColor)
{
     ...
     switch(Display.cfg.cfg1.language)
	{
		case DISP_LAN_ENGLISH:
			...
			break;
        case DISP_LAN_GERMANY:
			sprintf((char *)Config_buff,"UV-TAUSCH:");
			break;
        case DISP_LAN_FRENCH:
			sprintf((char *)Config_buff,"CHANGEMENT UV:");
			break;
        case DISP_LAN_SPANISH:
			sprintf((char *)Config_buff,"DURACION UV:");
			break;
        case DISP_LAN_ITALIAN:
			sprintf((char *)Config_buff,"DURATA UV:");
			break;
        case DISP_LAN_PORTUGUESE:
			sprintf((char *)Config_buff,"VIDA UV:");
			break;
		default:
    ...
    if (Display.ucCurCol < UV_SET_NUMS)
    {
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        case DISP_LAN_FRENCH:
            sprintf((char *)Config_buff,UV_FORMAT_FRE,Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_SPANISH:
            sprintf((char *)Config_buff,UV_FORMAT_SPA,Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_ITALIAN:
            sprintf((char *)Config_buff,UV_FORMAT_ITA,Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_PORTUGUESE:
            sprintf((char *)Config_buff,UV_FORMAT_POR,Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        }
    ...
    else
    {
        ...
        {
        case DISP_LAN_ENGLISH:
            sprintf((char *)Config_buff,"%dD %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_GERMANY:
            sprintf((char *)Config_buff,"%dT %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_FRENCH:
            sprintf((char *)Config_buff,"%dJ %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_SPANISH:
            sprintf((char *)Config_buff,"%dD %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_ITALIAN:
            sprintf((char *)Config_buff,"%dG %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
        case DISP_LAN_PORTUGUESE:
            sprintf((char *)Config_buff,"%dD %dHr    ",Display.cfg.cfg2.UVLIFEDAY,Display.cfg.cfg2.UVLIFEHOUR);
            break;
...
void Disp_engSetDrawRecirculation(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_SPANISH:
        ...
        break;
    case DISP_LAN_ITALIAN:
		sprintf((char *)Config_buff,"RICIRCOLAZIONE:%d min   ",Display.cfg.cfg1.cirtime);
        break;
    case DISP_LAN_PORTUGUESE:
		sprintf((char *)Config_buff,"RECIRC.    :%d min      ",Display.cfg.cfg1.cirtime);
        break;
...
void Disp_engSetDrawUFDisinfection(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_GERMANY:
        ...
        break;
    case DISP_LAN_FRENCH:
        sprintf((char *)Config_buff,"UF DESINFECTION:%d DAYS    ",Display.cfg.cfg3.usUFCleanTime);
        break;
    case DISP_LAN_SPANISH:
        sprintf((char *)Config_buff,"UF DESINFECCION.:%d TAGE      ",Display.cfg.cfg3.usUFCleanTime);
        break;
    case DISP_LAN_ITALIAN:
        sprintf((char *)Config_buff,"UF DISINFEZIONE:%d DAYS    ",Display.cfg.cfg3.usUFCleanTime);
        break;
    case DISP_LAN_PORTUGUESE:
        sprintf((char *)Config_buff,"UF DESINFECCAO:%d TAGE      ",Display.cfg.cfg3.usUFCleanTime);
        break;    
    default:
...
void Disp_engSetDrawUFFlush(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_ENGLISH:
        ...
        break;
    case DISP_LAN_GERMANY:
        sprintf((char *)Config_buff,"SPUELUNG :%d sec      ",Display.cfg.cfg3.usUFFlushTime);
        break;
    case DISP_LAN_FRENCH:
        sprintf((char *)Config_buff,"UF LAVAGE:%d sec      ",Display.cfg.cfg3.usUFFlushTime);
        break;
    case DISP_LAN_SPANISH:
        sprintf((char *)Config_buff,"ENJUAGUE UF:%d sec      ",Display.cfg.cfg3.usUFFlushTime);
        break;
    case DISP_LAN_ITALIAN:
        sprintf((char *)Config_buff,"RISCIAQUO:%d sec      ",Display.cfg.cfg3.usUFFlushTime);
        break;
    case DISP_LAN_PORTUGUESE:
        sprintf((char *)Config_buff,"UF NIVELADO:%d sec      ",Display.cfg.cfg3.usUFFlushTime);
        break;    
    default:
...
void Disp_engSetDrawUFAutoFlush(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    case DISP_LAN_GERMANY:
        ...
        break;
    case DISP_LAN_FRENCH:
        sprintf((char *)Config_buff,"AUTO. LAVAGE :%d Min      ",Display.cfg.cfg3.usAutoUFFlushTime);
        break;
    case DISP_LAN_SPANISH:
         sprintf((char *)Config_buff,"ENJUAGUE AUTOMATICO:%d Min",Display.cfg.cfg3.usAutoUFFlushTime);
        break;
    case DISP_LAN_ITALIAN:
        sprintf((char *)Config_buff,"RISCIAQUO AUTOMATICO:%d Min",Display.cfg.cfg3.usAutoUFFlushTime);
        break;
    case DISP_LAN_PORTUGUESE:
        sprintf((char *)Config_buff,"LAVE AUTO :%d Min      ",Display.cfg.cfg3.usAutoUFFlushTime);
        break;    
    default:
    ...
    case KEY_CODE_RECURSIVE_K2:
		...
		{    
            ...
            if (KEYSTATE_PRESSED == state)
            {
				...
				/*------------关闭UF提醒------------*/
				else if (((1<<KEY_CODE_RECURSIVE_K2)|(1<<KEY_CODE_SET_K4)) == ls)
			    {
...
/*void Disp_KeyHandler_toc_idle(int key,int state) //20160926ìí?ó
{
    switch(key)
    {
    case KEY_CODE_NORMAL_K1:
...
{
   uint8_t lan[] = {DISP_LAN_CHINESE,DISP_LAN_GERMANY,DISP_LAN_FRENCH,
		             DISP_LAN_SPANISH,DISP_LAN_ITALIAN,DISP_LAN_PORTUGUESE,
		             DISP_LAN_ENGLISH}; 
...
void Disp_DisplayUFCleanSettingPage(void)
{
		...
		case DISP_LAN_FRENCH:
			  {
                ...
                text = "CleanPack et tablette";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;            
    
                text = "apres poussee le bouton";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }            
			break;
        case DISP_LAN_SPANISH:
			{
                ...
                text = "Clenpack y pastilla";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;            
    
                text = "despues presionar el";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
				yoff += curFont->sizeY + 5; 
        ...
        case DISP_LAN_ITALIAN:
			{
                ...
                text = "Cleanpack e pastiglia";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;            
    
                text = "dopo premere il tasto";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }            
			break;
        case DISP_LAN_PORTUGUESE: 
			{
                ...
                text = "Cleanpack e comprimidos";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;            
    
                text = "depois que a tecla push";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }            
			break;
        default:
    ...
    case DISPLAY_SUB_PAGE_US_SET_UF_CLEAN_STEP2:
		switch(Display.cfg.cfg1.language)
		{
			...
			case DISP_LAN_GERMANY:
			{
                text = "Neuen Pack einsetzen,";
                xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff = LCD_H/2  - 6 - curFont->sizeY;
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
                text = "dann Taste-druchen";
               //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
               yoff += curFont->sizeY + 5;
               curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }
			...
			case DISP_LAN_FRENCH:
			{
                ...
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;           
                text = "un nouveau bloc et apres";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;                
                text = "cett poussee le bouton";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }
			    break;
			case DISP_LAN_SPANISH:
			{
                ...
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;           
                text = "nuevo paquete despues";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;                
                text = "presionar el boton";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
			...
			case DISP_LAN_ITALIAN:
			{
                ...
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;           
                text = "nuovo pack e dopo";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;                
                text = "premere il tasto";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }
			    break;
			case DISP_LAN_PORTUGUESE:
			{
                ...
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;           
                text = "novo pacote depois";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);    
                //xoff = LCD_W/2 - curFont->sizeX/2*(strlen(text)+1);
                yoff += curFont->sizeY + 5;                
                text = "essa chave push-";
                curFont->DrawText(xoff,yoff,(uint8_t *)text,Display.usForColor,Display.usBackColor);
            }
			    break;
			default:
...
void Disp_ResetInfo(void) //ó?óú??3yó???á?20161018
{
    ...
    if (Display.ucCurCol < PACK_SET_NUMS)
    {
...

        {
          ...
          case DISP_LAN_GERMANY:
              ...
              break;
          case DISP_LAN_FRENCH:
              sscanf((char *)Config_buff,PACK_FORMAT_FRE,&max,&min);
              break;
          case DISP_LAN_SPANISH:
              sscanf((char *)Config_buff,PACK_FORMAT_SPA,&max,&min);
              break;
          case DISP_LAN_ITALIAN:
              sscanf((char *)Config_buff,PACK_FORMAT_ITA,&max,&min);
              break;
          case DISP_LAN_PORTUGUESE:
              sscanf((char *)Config_buff,PACK_FORMAT_POR,&max,&min);
              break;
    ...
    if (Display.ucCurCol < UV_SET_NUMS)
    {
...

        {
          ...
          case DISP_LAN_GERMANY:
              ...
              break;
          case DISP_LAN_FRENCH:
              sscanf((char *)Config_buff,UV_FORMAT_FRE,&max,&min);
              break;
          case DISP_LAN_SPANISH:
              sscanf((char *)Config_buff,UV_FORMAT_SPA,&max,&min);
              break;
          case DISP_LAN_ITALIAN:
              sscanf((char *)Config_buff,UV_FORMAT_ITA,&max,&min);
              break;
          case DISP_LAN_PORTUGUESE:
              sscanf((char *)Config_buff,UV_FORMAT_POR,&max,&min);
              break;
          }
...
void Disp_ReadConfig(void)
{
   ...
   /*
	...
	sprintf((char *)Display.cfg.cfg3.SERIALNO,"M2003B0303");
...
#define OFFSET_TIME 100                        
        ...
        if (DISPLAY_STATE_IDLE == Display.CurState.ucMain)
        {
            ...
            if ((Display.ulIdleFlushTime >= (Display.cfg.cfg3.usAutoUFFlushTime*60))
				&&(Display.cfg.cfg3.usAutoUFFlushTime > 0))
            {
                if (DISPLAY_PAGE_IDLE == Display.curPage.ucMain
                    && DISPLAY_SUB_PAGE_IDLE == Display.curPage.ucSub)
                {
            ...
            {
               // LCD_EnableBackLight(FALSE); //一段时间误操作，LCD背光关闭
                LCD_EnableBackLight(TRUE);  //LCD背光常开
                Display.usIdleLcdTimes = 0;
            }
        }     
...
void Disp_Adc_proc(void)
{
  ...
  if (adc)
  {
      ...
      }
      {
                             ...
                             | (1 << DISPLAY_STATE_IDLE)))//09  27 
                {
                    ...
                    if (!(flags & (1 << DISPLAY_STATE_TOC)))
                    {
                        if (DISPLAY_SENSOR_2 == iChl)
                        {
                            ...
                            if ((Display.iConductivity[DISPLAY_SENSOR_2]/100) > Display.cfg.cfg2.PROD_RES[1]) //???????è±¨?ˉì??t
                            {
        ...
        /* if(iChl == 2)
         	{
         	   UartCmdPrintf(VOS_LOG_DEBUG,"CHL%d,C:%dmv,T:%dmv S:%d \n",iChl+1,temp_average_b,temp_sum_p,Display.iConductivity[iChl]);
         	}
		 else */
		 	UartCmdPrintf(VOS_LOG_DEBUG,"CHL%d,C:%dmv,T:%dmv \n",iChl+1,value1,value2);
...
uint8 Disp_DataHandler(uint8 *pCmd,uint8 *pRsp,uint8 CmdLen,uint8 *pucRspLen)
{
    ...
    case RF_CMD_RESET_CM:  //?′??o?2?
        {
            if (Display.bit1AuthState)
            {
                uint32_t ulMask = BUILD_UINT32(pData[0],pData[1],pData[2],pData[3]);

				//Disp_ResetInfo();//ó???á?éè?a0
                //Storage_UpdateUser(&Display.UsrInfo);//??ó??§ó???á??üD??a0
...
void Disp_Init(void)
{
    ...
    Display.ausSampleMask[DISPLAY_SENSOR_1] =  (1<<DISPLAY_STATE_IDLE) 
                                               ...
                                               |(1<<DISPLAY_STATE_NORMAL_TAKING_WATER)
                                               |(1<<DISPLAY_STATE_QUANTITY_TAKING_WATER)
                                               |(0<<DISPLAY_STATE_TOC)   // 1 TOC1y3ì2??ì2aí¨μà1
   ...
   Display.ucChlMask |= 1<<DISPLAY_SENSOR_2; 
    ...
    Display.ausSampleMask[DISPLAY_SENSOR_TOC] = (0<<DISPLAY_STATE_IDLE)   //0
                                               |(1<<DISPLAY_STATE_CIRCULATION)
                                               |(0<<DISPLAY_STATE_NORMAL_TAKING_WATER)// 2ú??ê±2??ì2aí¨μà3
                                               |(0<<DISPLAY_STATE_QUANTITY_TAKING_WATER) // 2??ì2aí¨μà3
