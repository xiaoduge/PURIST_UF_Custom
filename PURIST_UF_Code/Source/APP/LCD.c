...
#else
/*20170208修改进水水质字体放大，修改LCD_PutChar24x48驱动函数*/
void LCD_PutChar24x48(u16 x,u16 y,u8 c,u16 charColor,u16 bkColor)
{
    u16 i=0;
    u16 j=0;
    ...
    { 
        ...
        {    
            ...
            {   
				...
				for(col = 0; col < 6;col++)
				{
                    unsigned char m =pgm_read_byte(&codeGB_48[k].Msk2[i*6 + col]);
                    for(j=0;j<8;j++) 
                    {
                        if((m&0x80)==0x80) 
                        {
                            LCD_SetPoint(x+j+col*8,y+i,fColor);
                        }
                        else 
                        {
                            LCD_SetPoint(x+j+col*8,y+i,bColor); 
                        }
                        m<<=1;
                    } 
				}
